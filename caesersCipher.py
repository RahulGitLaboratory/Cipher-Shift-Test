...
def encrypt(message):
    ...
    # encrypted_message = "".join([str(idx) for idx, char in enumerate(self.my_message)])
    encrypted_message = "".join([abc[abc.find(char) + 1] if len(abc) > (abc.find(char) + 1) else
                                 abc[0] for idx, char in enumerate(message)])
...
class TestEncryption(unittest.TestCase):
    ...
    def test_shiftedCipher(self):
        ...
        # encrypted_message = "".join([str(idx) for idx, char in enumerate(self.my_message)])
        encrypted_message = "".join([abc[abc.find(char) + 1] if len(abc) > (abc.find(char) + 1) else
                                     abc[0] for idx, char in enumerate(self.my_message)])
